...
std::unordered_map<int,int> indexation;
...
std::array<int,2> PyramidScheme(int node){
    ...
    if(size > 2){
        for(int i=1; i < size-1; i++){
            ...
            size2 = tree[indexation[tree[index][i]]].size();
            if(size2 > 2){
                for(int j=1; j < size2-1; j++){
                    pre_solution2 = PyramidScheme(tree[indexation[tree[index][i]]][j]); 
        ...
        return {bestnodes + 1, bestcosts + selfcost};
    }
    else{
        return {1, selfcost};
    }
